...
export const Profesionales2: React.FC = () => {
  ...
  const filteredProfesionales2 = profesionalesData.filter(p => 
    p.hacedomicilio === domicilio ||
    p.nombre.toLowerCase().includes(busqueda.toLowerCase()) ||
    p.especialidad.toLowerCase().includes(busqueda.toLowerCase()) ||
    p.ubicacion.toLowerCase().includes(busqueda.toLowerCase())
  );
  ...
  return (
    <Container className="py-4">
      ...
      <InputGroup className="mb-4">
        ...
        <SwitchExample etiqueta={'A domicilio'} activo={domicilio} setActivo={setDomicilio} />
      </InputGroup>

      {/* Grid de Profesionales */}
      <Row xs={1} md={2} lg={3} className="g-4">
        {filteredProfesionales2.map((prof) => (
          <Col key={prof.id}>
            <Card className="h-100 shadow-sm hover-card">
              <Card.Body>
                <div className="d-flex align-items-center mb-3">
